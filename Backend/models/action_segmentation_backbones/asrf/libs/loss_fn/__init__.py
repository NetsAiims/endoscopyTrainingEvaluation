...
class ActionSegmentationLoss(nn.Module):
    ...
    def __init__(
        ...
        use_probability_vector: bool = False
    ) -> None:
        super().__init__()
        self.criterions = []
        self.weights = []
        self.use_probability_vector = use_probability_vector
    ...
    def forward(
        ...
        for criterion, weight in zip(self.criterions, self.weights):
            if self.use_probability_vector:
                preds = preds.type(torch.float)
                gts = torch.swapaxes(gts, 1, 2).type(torch.float)
