...
def get_class_nums(
    ...
    if epoch == -1:
        ...
        if mode == "training":
            df = pd.read_csv(os.path.join(csv_dir, dataset, str(epoch), "train{}.csv").format(split))
        elif mode == "trainval":
            df1 = pd.read_csv(os.path.join(csv_dir, dataset, str(epoch), "train{}.csv".format(split)))
...
def get_class_weight(
    ...
    nums = get_class_nums(dataset, split, dataset_dir, csv_dir, mode, epoch)
