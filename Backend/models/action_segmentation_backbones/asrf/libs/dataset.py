...
from libs.class_id_map import read_dataset_config
...
class ActionSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        csv_dir: str = "./csv",
        epoch: int = -1,
        epoch_variational_gt: bool = False
    ) -> None:
        super().__init__()
        """
            Args:
                ...
                epoch: specify the number of epoch for augmentation to apply. Incase of no augmentation use -1.
        ...
        if epoch == -1:
            ...
            if mode == "training":
                self.df = pd.read_csv(
                    os.path.join(csv_dir, dataset, str(epoch), "train{}.csv".format(split))
                )
            elif mode == "validation":
                self.df = pd.read_csv(
                    os.path.join(csv_dir, dataset, str(epoch), "val{}.csv".format(split))
                )
            elif mode == "trainval":
                df1 = pd.read_csv(
                    os.path.join(csv_dir, dataset, str(epoch), "train{}.csv".format(split))
                )
                df2 = pd.read_csv(os.path.join(csv_dir, dataset, str(epoch), "val{}.csv".format(split)))
                self.df = pd.concat([df1, df2])
            elif mode == "test":
                self.df = pd.read_csv(
                    os.path.join(csv_dir, dataset, str(epoch), "test{}.csv".format(split))
                )
            else:
                assert (
                    mode in modes
                ), "You have to choose 'training', 'trainval', 'validation' or 'test' as the dataset mode."
        print(mode, len(self.df))
    ...
    for s in sample:
        ...
        if pad_t > 0:
            ...
            if label.dtype == np.float32:
                label = F.pad(label, (0, pad_t), mode="constant", value=0.0)
            else:
